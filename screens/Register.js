...
import {emailValidator,passwordValidator, phoneValidator, nameValidator, addressValidator, usernameValidator ,tradingExperienceYearsValidator} from '../componants/helpers/dataValidator';
...
export default function Register({ navigation,route }) {
  ...
  const validateStep2 = () => {
    ...
    settradingExperienceYears({ ...tradingExperienceYears, error: tradingExperienceYearsError });
...
if(step === 3)
{
 ...
 const sendDataToBackend = async () => {
  try {
    ...
    formData.append('tradingExperienceYears', tradingExperienceYears.value.toString());
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <KeyboardAwareScrollView contentContainerStyle={styles.container}>
        <Background>
          ...
          {/* <Logo /> */}
          <Motherlogo />
          {/* <Header>Register as {RoleType}</Header> */}
          {renderProgressBar()}
          {step === 1 && (
            <View>
              <TextInput
...
<View style={{ alignItems: 'center', marginTop: 20, flexDirection: 'row', alignSelf: 'center' }}>
  <TouchableOpacity onPress={() => setShowDatePicker(true)}>
    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
      <Icon name="calendar" size={15} color="#000" />
      <Text style={{ marginLeft: 10 }}>{labels.birthDate}</Text>
    </View>
  </TouchableOpacity>
...
{hasError && (
  <View style={{ alignItems: 'center', marginTop: 10 }}>
    <Text style={{ color: 'red' }}>{labels.selectBirthDate}</Text>
  </View>
)}
...
{formattedDate && (
  ...
  <View style={styles.imageContainer}>
    {selectedImage && ( // Check if selectedImage is not null
      <Image source={{ uri: selectedImage }} style={styles.image} />
    )}
    {!selectedImage && ( // Check if selectedImage is null
      <TouchableOpacity
        style={styles.uploadButton}
        onPress={handleImageUpload}
      >
        <Text style={styles.uploadButtonText}>{labels.uploadImage}</Text>
      </TouchableOpacity>
    )}
  </View>
)}
